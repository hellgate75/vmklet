...
func (Command URLCommand) Parse(arguments ...string) (model.Command, error) {
	...
	var parsedCommand string = fmt.Sprintf("%v", model.URLCommand) + " " + strings.Join(arguments, " ")
	if len(arguments) > 1 {
		for index, argument := range arguments {
			if index%2 == 0 {
				...
				if index >= len(arguments)-1 {
					return nil, errors.New(fmt.Sprintf("URL Command '%s' has uncompleted argument '%s'!!", parsedCommand, key))
				}
				...
				if strings.Index(utils.CorrectInput(key), "ssl") >= 0 {
					...
					if format != "http" {
						return nil, errors.New(fmt.Sprintf("URL Command '%s' key '%s' not provided!!", parsedCommand, format))
					}
				} else if strings.Index(utils.CorrectInput(key), "timeout") >= 0 {
					timeout, err = utils.StringToInt(value)
					if err != nil {
						return nil, errors.New(fmt.Sprintf("URL Command '%s' error parsing timeout argument '%s'!!", parsedCommand, value))
					}
				} else if strings.Index(utils.CorrectInput(key), "code") >= 0 {
					expectedCode, err = utils.StringToInt(value)
					if err != nil {
						return nil, errors.New(fmt.Sprintf("URL Command '%s' error parsing return code argument '%s'!!", parsedCommand, value))
					}
				}
			}
		}
	...
	if format == "" {
		return nil, errors.New(fmt.Sprintf("URL Command '%s' fromat argument is mandatory!!", parsedCommand))
	}
	return &URLCommand{
